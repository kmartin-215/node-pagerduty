...
let options = (url, method, key, qs, body) =>{
    let replacedElements = replaceArrayElements(qs);
    let parsedQs = replacedElements.querystring;
    delete replacedElements['querystring'];
    qs = replacedElements ? '?' + querystring.stringify(replacedElements) + (parsedQs ? '&' + parsedQs : '') : '';
    let _option =  {
        uri: baseUrl + url + qs,
        method: method,
        headers: {
...
// Functions
//
function replaceArrayElements(qs) {
    let replaceElementNames = ['include','escalation_policy_ids','schedule_ids','user_ids']
    let elements = [];
    let element = null;

    for (var key in qs) {
        if (replaceElementNames.includes(key)) {
            element = eval('qs.' + key);
            delete qs[key];
            for (var i = 0, len = element.length; i < len; i++) {
                let pushval = key + '[]=' + element[i];
    ...
    if (elements !== undefined && elements.length > 0) {
        newQs = newQs + elements.join('&');
    }
    qs.querystring = newQs;
    return qs;
}
